...
echo "${bold}${magenta}creating firewall rules...${reset}"
...
gcloud -q ompute firewall-rules create deny-tcp \
--network vpc \
--action deny \
--rules tcp \
--target-tags db-tier \
...
echo "${bold}${magenta}creating web server instances...${reset}"
...
echo "${bold}${magenta}creating db server instances...${reset}"
...
echo "${bold}${magenta}creating instance groups and named ports...${reset}"
...
echo "${bold}${magenta}creating health check...${reset}"

gcloud -q compute health-checks create tcp my-tcp-health-check --port 110

echo "${bold}${magenta}creating backend services...${reset}"
...
echo "${bold}${magenta}adding instance groups to backend service...${reset}"
...
echo "${bold}${magenta}creating tcp proxy...${reset}"

gcloud -q compute target-tcp-proxies create my-tcp-lb-target-proxy \
--backend-service my-tcp-lb \
--proxy-header NONE

echo "${bold}${magenta}reserve forwarding rule address...${reset}"

gcloud -q compute addresses create tcp-lb-static-ipv4 \
--ip-version=IPV4 \
--global

echo "${bold}${magenta}creating forwarding rule...${reset}"

gcloud beta -q compute forwarding-rules create my-tcp-lb-ipv4-forwarding-rule \
--global \
--target-tcp-proxy my-tcp-lb-target-proxy \
--address ${LB_STATIC_IPV4} \
--ports 110

echo "${bold}${magenta}creating project metadata to store tcp procy address...${reset}"

gcloud -q compute project-info add-metadata \
--metadata lb-vip=$LB_STATIC_IPV4

echo "${bold}${magenta}creating a probe instance...${reset}"
